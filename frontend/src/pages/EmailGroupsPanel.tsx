import { Alert, Button, Checkbox, FormControlLabel, Paper, Stack, Table, TableBody, TableCell, TableHead, TablePagination, TableRow, TextField, Typography } from '@mui/material';
...
export default function EmailGroupsPanel({onChange}:{onChange?:()=>void}){
  ...
  return <Stack spacing={2}>
    ...
    <Table size="small"><TableHead><TableRow><TableCell>Grupo</TableCell><TableCell>Descrição</TableCell><TableCell>Remetentes</TableCell><TableCell>Ações</TableCell></TableRow></TableHead><TableBody>{grupos.filter(g=>Boolean(g.ativo)).slice(page*pageSize,page*pageSize+pageSize).map(g=><TableRow key={g.id}><TableCell>{g.nome}</TableCell><TableCell>{g.descricao||'-'}</TableCell><TableCell>{Number(g.quantidade||0)}</TableCell><TableCell><Button size="small" onClick={()=>editar(g)}>Editar</Button><Button size="small" color="error" onClick={()=>desativar(g)}>Desativar</Button></TableCell></TableRow>)}</TableBody></Table>
    <TablePagination component="div" count={grupos.filter(g=>Boolean(g.ativo)).length} page={page} rowsPerPage={pageSize} onPageChange={(_,p)=>setPage(p)} onRowsPerPageChange={e=>{setPageSize(Number(e.target.value));setPage(0)}} rowsPerPageOptions={[25,50,100,250,500]}/>
  </Stack>;
}
